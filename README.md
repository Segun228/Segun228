# Hi there, I'm Nestor 👋

**🎯 Fullstack Analyst | 🧠 Data Scietist | 📊 Analyst-Developer**  
**📍 Moscow | 📧 segunperkele@gmail.com | [Telegram](https://t.me/dianabol_metandienon_enjoyer)**

## 👨‍💻 About Me

I'm a results-driven product and data analyst with 1,5+ years of experience building scalable digital solutions with strong backend and ML foundations. My expertise spans from product analytics and data-driven development to solving complex business challenges through technology and machine learning. With a unique blend of product thinking, backend and ML development experience, and BI implementation skills, I transform business problems into data-informed solutions — driving efficiency, growth, and measurable impact.

## 🚀 Current Roles

### 🏦 Data/Product Analyst @ HSE (Higher School of Economics) | *Sep 2024 – Present*
- Working on data-driven initiatives for a large-scale fintech ecosystem
- Building analytical models to support business decisions  
- Collaborating with cross-functional teams to deliver impactful insights and improve financial services
- Searching insights in data and testing business decisions via data-driven ideology

#### 🔁 Process Optimization  
- Automated data processing workflows — reducing manual reporting time by **60%**
- Streamlined data collection and analysis processes, improving efficiency
- Implemented automated monitoring systems that reduced error rates by **25%**

#### 📊 Business Intelligence & Analytics
- Built **10+ interactive dashboards** in Grafana, Superset, and Power BI
- Developed predictive models that improved business decision accuracy by **35%**
- Created real-time KPI tracking systems that reduced reporting latency by **40%**

#### 🤝 Product & Data Strategy
- Integrated data analytics into product development lifecycle
- Established data-driven feedback loops that increased product adoption by **40%**
- Collaborated with engineering teams to implement tracking and analytics infrastructure

## 🎓 Education

**Bachelor's in Computer Science**  
*[HSE] | [2024] - [2028]*

*Relevant coursework:*
- Data Structures and Algorithms
- Machine Learning and Statistical Analysis
- Database Systems and Big Data Technologies
- Business Intelligence and Data Visualization

## 🛠 Technical Skills

**Data Analysis & BI** – Python, SQL, Pandas, Jupyter, Statistical Analysis  
**Data Visualization** – Grafana, Apache Superset, Power BI, Tableau  
**Backend Development** – Python, FastAPI, PostgreSQL, Docker  
**Data Engineering** – ETL Pipelines, Data Modeling, API Integration  
**Business Analytics** – KPI Development, A/B Testing, Product Metrics  
**Tools & Platforms** – Git, Docker, PostgreSQL, Excel, VS Code

## 🌍 Languages

- **English** – Professional Proficiency
- **Russian** – Native

## 📬 Let's Connect

I'm passionate about building data-driven products and solving complex business challenges through analytics and technology. Always open to discussing innovative projects, collaboration, or data challenges.

[![Telegram](https://t.me/dianabol_metandienon_enjoyer)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Segun228)
📧 **Email**: segunperkele@gmail.com
